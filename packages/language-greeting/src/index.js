...
async function handleRequest(request) {
  ...
  if (userAgent.includes("Googlebot")) {
    greetingContent =
      "HELLO GOOGLEBOT! 🤖<br> Is this cloaking? You decide! (please rank my site better)";
  } else {
    ...
    const languageGreeting =
      languageGreetings[browserLanguage] || languageGreetings["default"];
    const locationGreeting =
      locationGreetings[country] || locationGreetings["default"];
  ...
  const targetH1 =
    '<h1 class="alignwide wp-block-heading" style="margin-bottom:var(--wp--preset--spacing--60)">Technical SEO &amp; Software Dev.</h1>';

  const modifiedHtml = html.replace(targetH1, eyebrowHtml + targetH1);
